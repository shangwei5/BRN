...
from heavy_DeblurDataset import prepare_data_heavyrain, Dataset
from utils import *
import re
...
from generator import Generator_prelstm, print_network
...
parser.add_argument("--outf", type=str, default="logs/CRN/R100H", help='path of log files')
...
def main():
    ...
    net = Generator_prelstm(recurrent_iter=opt.inter_iter, use_GPU=opt.use_GPU)
...
#            tv_loss = tv(out_train)
            ...
            loss = -(pixel_loss + 0.5 * (loss1 + loss2 + loss3 + loss4)) #/(target_train.size()[0] * 2)
            loss_r = -(pixel_loss_r + 0.5 * (loss1_r + loss2_r + loss3_r + loss4_r))
            ...
            print("[epoch %d][%d/%d] loss: %.4f, PSNR_train: %.4f" %
                  (epoch + 1, i + 1, len(loader_train), lossm.item(), psnr_train))
            # if you are using older version of PyTorch, you may need to change loss.item() to loss.data[0]

            if step % 10 == 0:
                # Log the scalar values
                writer.add_scalar('loss', lossm.item(), step)
...
if __name__ == "__main__":
    if opt.preprocess:
        prepare_data_heavyrain(data_path=opt.data_path, patch_size=100, stride=100, aug_times=1)
